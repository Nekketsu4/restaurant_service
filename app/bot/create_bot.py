...
dp = Dispatcher(storage=MemoryStorage()) # Для боевых проектов я обычно использую хранилище Redis а не MemoryStorage
...
async def start_bot():
    ...
    if settings.INIT_DB:
        await init_db()
    setup_dialogs(dp)   # Это очень важная строка! Если вы ее не укажете, то возможности использовать aiogram dialog в проекте у вас не будет. Функция setup_dialogs регистрирует все необходимые мидлвари и обработчики для работы диалогов.
